      ...
      body: Container(
        ...
        child:ListView.builder(
          ...
          itemBuilder: (context, index) {
            if(AudioPlayerUtil.musicModel!.id == AudioPlayerUtil.list[index].id) {
              _isPlaying = true;
            }else{
              _isPlaying = false;
            }
            return ListTile(
              ...
              subtitle: Text(AudioPlayerUtil.list[index].author, style: const TextStyle(fontSize: 12),),
              trailing: IconButton(
                onPressed: (){
                  AudioPlayerUtil.removeMusicModel(model: AudioPlayerUtil.list[index]);
                  setState(() {});
                },
                icon:const Icon(Icons.clear_outlined,size: 18,color: Colors.grey,),
              ),
              onTap: (){
